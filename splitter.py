...
class SplitterTrainer(object):
    ...
    def process_walk(self, walk):
        ...
        """
        left_nodes = [walk[i] for i in range(len(walk) - self.args.window_size) for j in
                      range(1, self.args.window_size + 1)]
        right_nodes = [walk[i + j] for i in range(len(walk) - self.args.window_size) for j in
                       range(1, self.args.window_size + 1)]
    ...
    def save_embedding(self):
        ...
        base_embedding = pd.DataFrame(self.base_node_embedding)
        base_embedding.to_csv(self.args.embedding_output_path, index=False)
